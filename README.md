...
- **账号分组（多账号池）** —— 页面顶部按分组查看与切换账号：默认分组就是升级前那套；
  点「添加分组」登记新的分组——**只填名称也行**（上游地址默认带出默认分组的地址，
  账号目录默认带出建议路径）。每个分组 = 一套独立的上游实例，按需把地址、`api_key`
  与**账号目录**改成那套实例的（账号目录一致才能在面板里管理这一组的账号；地址保持
  与默认分组一致时页面上会说明「共用同一套实例、搬进来的账号不会被实际使用」）。账号行有「移动到分组」，
